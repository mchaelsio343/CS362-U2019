...
public class UrlValidatorTest extends TestCase {
   private final String testFileRelPath = "/test/";
...
//   }
   ...
   public void testUnit() {
	   ...
	   try(BufferedReader in = new BufferedReader(new FileReader(filePath + testFileRelPath + testFile))) {
		    String str;
		    while ((str = in.readLine()) != null) {
		    	// read line
		    	String[] tokens = str.split(",");
		    	
		    	// extract url and boolean value
		    	url = tokens[0];
		    	expected = Boolean.parseBoolean(tokens[1]);
   ...
   public void testRandom() {
	   ...
	   for (int i = 1 ; i <= RUN_TIMES ; i++) {
		   ...
		   for(int testUrlPartsIndex = 0; testUrlPartsIndex < testUrlParts.length ; testUrlPartsIndex++){
			   // iterate all url parts 
			   ResultPair[] singleTestUrlPart = (ResultPair[]) testUrlParts[testUrlPartsIndex];
